...
def vpn_sync(client_socket, vpn_client_buff, addr, port, server_con_status, client_status_close, competing):
    ...
    # 关闭客户端连接
    if competing[0] == 'vpn' or competing[0] == 'dont_competing':
        if debug == True:
            logging.info('%s: 代理：关闭客户端连接' % addr)
...
def con_sync(client_socket, con_client_buff, addr, port, server_con_status, client_status_close, competing):
    ...
    # 关闭客户端连接
    if competing[0] == 'con' or competing[0] == 'dont_competing':
        if debug == True:
            logging.info('%s: 直连：关闭客户端连接' % addr)
            connect_cont['client_cont'] = connect_cont['client_cont'] - 1
        try:
            client_socket.close()
        except:
...
def handle_client(client_socket):
    ...
    # 静态模式
    if static_mode == True and route_jg == 'none':
        try:
            # 解析ip
            resolver = dns.resolver.Resolver()
            resolver.nameservers = static_dns
            ip_addr = resolver.resolve(addr)[0]
            ...
            if route_jg == 'con' or route_jg == 'vpn':
                competing.append('dont_competing')

        except:
            route_jg = 'none'
            if debug == True:
                logging.info("%s: 静态模式：失败，切换为动态模式"% addr)
    ...
    while True:
        try:
            scs = server_con_status.get(timeout=10)
        except:
            scs = False
    ...
    if len(competing) == 0:
        if debug == True:
            logging.info('%s: 中央：关闭客户端连接' % addr)
            connect_cont['client_cont'] = connect_cont['client_cont'] - 1
...
if __name__ == "__main__":
    ...
    if os.path.exists('config.yaml') == False:
        config = {
            ...
            "debug": False,
            "static_mode": True,
            "static_dns": ["119.29.29.29", "114.114.114.114"],
            "static_route_url": 'https://raw.githubusercontent.com/17mon/china_ip_list/master/china_ip_list.txt',
            "route_timeout": 1800,
            "lag_time": 300,
            'reject': [],
            "local": [
    ...
    static_route_url =config["static_route_url"]
    ...
    if static_mode == True:
        ...
        try:
            respon = requests.get(static_route_url, proxies=proxyd)
            respon_data = respon.text.strip().strip('\r').split('\n')
            if len(respon_data) != len(subnet_list) and len(respon_data) > 5000:
                subnet_list = respon_data
                if debug == True:
                    logging.info('远程IP列表更新，使用远程列表')
            else:
                if debug == True:
                    logging.info('使用本地IP列表')

        except:
            if debug == True:
                logging.warning('远程ip列表拉取失败，使用本地列表')
            pass
